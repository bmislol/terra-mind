...
**Status:** **Phase 6.3 complete — Section 6 done (graded core: guardrails + red-team gate + CI eval gates proven; Sections 1–6 shipped).** Tenant isolation is now a **CI-gated fact**, not a claim. 4.1a built backend auth (`fastapi-users` argon2id user model on `tenants`, no migration; custom access(30 m)+refresh(30 d) JWTs; register/login/refresh/logout/guest; Redis-denylist revocation by `jti`, D-029; `/bot/ask` access-JWT gate; RLS context mechanism + **D-030** NULLIF fail-closed). **4.1b delivered:** short-term memory wired into `/bot/ask` (`app/memory/short_term.py` N=20/TTL=2 h, **D-031**, redaction-on-write) via the **two-short-transactions** RLS pattern (`app/services/memory.py`: resolve_session → agent with no DB held → record_turn, each re-setting `set_tenant_context`); **tenant isolation PROVEN end-to-end** through the real `/bot/ask` path as non-superuser `terramind_app` (`tests/services/test_rls_isolation.py` — under B's context, zero of A's rows); **conversation/data erasure** `DELETE /me` (**D-032**) proven physical via the owner connection (`tests/api/test_erasure.py`); `auth.login`/`session.revoked`/`tenant.erased` audit. **248/248 tests green** (real Postgres via testcontainers + fakeredis). **4.2 delivered (first C#):** fresh tModLoader mod in `client/TerraMind/` — `/bot` `ModCommand` reads live `Main.LocalPlayer` gear/inventory/stats + world flags (`NPC.downed*`) into the exact `StatePayload` JSON (explicit `[JsonPropertyName]` snake_case; `item.type`→`item_id`). **No CI for the mod** (ARCH §13.3) — **verified in-game across 7 `/bot test` runs** via `client.log` (gear/weapon/armor-split/defense/HP/prefixes/shape all track the real character); RUNBOOK §10 has build+verify steps. **4.3 delivered (mod = full authed chat client):** `/bot login <user> <pass>` (form-encoded `/auth/jwt/login`, password in-memory then discarded — chat-command **not** config, since `ModConfig` auto-persists; config holds the backend URL only, D-027 4.3 revision) → token pair persisted to `token.json` under `Main.SavePath` (tokens-only, no password) → restored on launch with a world-entry confirmation → **`/auth/refresh` on launch and on a 401** mints a fresh access JWT (durable past the 30-min TTL; shared `AuthFlow`) → **`/bot logout`** deletes `token.json` + denylists server-side (D-029). Authed `/bot/ask` renders the real `AskResponse` (`answer`+`session_id`, session threaded); **Ranger detected through the live path**. **No CI** (ARCH §13.3) — verified in-game across all 3 commits, evidence in `client/VERIFICATION.md` (the no-CI gate); rebuild **0 errors/0 warnings** after correcting the 4.2 nullable claim (csproj `<Nullable>` never reached the tML compile → per-file `#nullable enable`). **P-016 logged:** agent under-uses the always-sent live state on generic progression questions (Section 3/6 prompt/router tuning). **4.4 delivered (Section-4 payoff — end-to-end in-game):** the production chat surface verified as designed — **both router paths** fire from in-game (`routing=faq` for an item-effect lookup, `routing=agent` for hard progression Qs), the agent grounds **reliably** in live state, and an in-game turn yields **one clean Langfuse trace** (`bot.ask → router → agent/faq → rag.retrieve + llm`, with real token counts). **P-016 FIXED** by a contained `agent_system.md` grounding instruction (ground in the tool-provided live state every turn; never ask for lookupable state — chosen over raw-state injection, which would undercut D-026) — in-game after-pass: **3/3** reliable grounding + class-distinct melee↔ranger answers; broad cross-question measurement deferred to Section 6 (eval harness). Verified in-game (no CI, ARCH §13.3); evidence in `client/VERIFICATION.md` §4.4. **Carried:** memory `get_history` built-not-consumed; `DELETE /me` data-only (account deletion P-015); the 4.1b Docker CI run must be green before its PR merges. **Open polish (none blocking):** P-009, P-010, P-012, P-013, P-014. (P-016 fix-landed 4.4; broad measurement → S6.) **5.1 delivered (Section 5 — Web Surfaces):** **Part A backend** — three CI-tested additions over the Section-4 backend: **locked-origin CORS** (never `*`, tested), **`GET /versions`** (public corpus metadata), and **`tenant_preferences`** with the fail-closed `tenant_isolation` RLS policy (D-030 form — the **second** tenant-scoped type under DB-enforced RLS; a separate table, not a `tenants` column, which can't carry fail-closed RLS without breaking the pre-auth login lookups) + `GET`/`PATCH /me/preferences` (**255 tests green**). **Part B portal** — the React config portal (`frontend-user/`, Vite+React+TS): login/register/guest, version dropdown, prefs GET/PATCH (persist across reload), erasure-with-confirm; **token-only `localStorage`**, `401→/auth/refresh`; **erasure hidden for guests**; nginx bundle on `:5173` (compose `frontend-user`, `depends_on api`; api `start_period` 5s→150s for a clean cold `up` — **proven** via `down`+`up --build`). **D-011 revised** (polished portal). **D-032 extended:** erasure **RETAINS** preferences (account config). **Two-client model documented** (ARCH §13.2: portal & mod = separate clients/logins; guest is portal-only). **P-017:** stored `selected_version` not yet consumed by `/bot/ask` retrieval (storage + round-trip only). Verified in-browser (register → version → prefs persist → erasure → re-login prefs survived; guest path). Design-token SKILL unavailable → standard React/CSS defaults. **5.2 delivered (Streamlit operator/test bench — Section 5 done):** **Backend** — operator **bootstrap** (`app/entrypoints/bootstrap_operator`, idempotent create-or-promote to `is_superuser`; makes RUNBOOK §3 honest) + two **operator-only cross-tenant views** `GET /admin/tenants` + `GET /admin/audit-log` (`require_operator`-gated, **no tenant context** — `tenants`/`audit_log` have no RLS, **D-017 generalized to two data categories**: per-tenant content = fail-closed RLS, admin cross-tenant = role-gated; verified against the migrations). **Deferred:** `/admin/versions/check` (P-018), `/admin/rerag` button (P-019 — `build_corpus.py` script is the must-have). **Bench** — `frontend-admin/` Streamlit on `:8501` (server-side calls → no CORS): operator-gated (player blocked); **test chat** (the RUNBOOK §7.1 demo fallback) with 3 presets carrying **real Cargo item_ids** (melee 3507 / ranger 98 / mage 518 — class-distinct answers verified e2e) → `/bot/ask` → answer + routing + session_id + raw payload; versions/tenants/audit views. **259 tests green.** Compose `frontend-admin` real service; api `start_period` 150→**240s** (third false-alarm fixed, cold-boot-proven). Functional-not-polished; **UI polish for both web surfaces deferred to a later pass** (acknowledged choice). **5.3 delivered (operator re-rag as a robust background job — reverses P-019 → D-033):** the re-rag *button* + the machinery behind it, in **4 staged commits** (worker/broker infra → `build_corpus` progress-seam refactor → API+job+guard → UI+verify). **RQ on the existing Redis** + a dedicated **`worker`** container (`rq worker rerag`, api image, `DATABASE_URL` + data volume RW + Redis, **no Vault**); the job runs `run_build(force=False)` — the idempotent upsert, **never `--force`** (which leaves a half-version on a mid-run death) — re-embedding the **cached** corpus (re-scrape = documented seam). **Single-job 409 guard** (Redis `SET NX` lock + heartbeated TTL); **streamed progress** (Redis live hash + the durable **`rerag_jobs`** row — operator/cross-tenant → **NO RLS**, `terramind_app` grant, `require_operator`-gated, **D-017**) polled by the Streamlit Versions tab (`st.fragment(run_every=2)`); `corpus.reragged` audit on success. **`run_build` relocated to `app/rag/corpus_build.py`** (the worker image ships `app/`, not `scripts/`; app→scripts would invert the layering — and the A-gate couldn't catch it since local tests have `scripts/` on path) + sync DSN fixed to **`postgresql+psycopg://`** (psycopg2 isn't a dep — the worker would've crashed at engine creation). **266 tests green.** **In-stack PROVEN with the REAL worker** (the unit tests faked `run_build`): operator `POST /admin/rerag` → worker embedded **5157/5157** pages (~5.5 min, progress streamed `300→5157`), **succeeded**, **`corpus.reragged`** audit landed, `rag_chunks`(1.4.4.9) = **22,173** queryable, 2nd concurrent → **409**. The live run **caught + fixed** RQ's 180s default `job_timeout` (→ 1800s) — the unit tests couldn't. **New deferrals:** P-020 (lock released before an RQ retry — single-worker-harmless), P-021 (worker/api re-download the embedding model on rebuild → share the HF cache; surfaced as an HF rate-limit boot stall during verify). **6.1 delivered (the GRADED CORE — guardrails + red-team gate, P-006 → D-034)** in **4 staged commits**: a **two-tier deterministic-first guardrail** on both `/bot/ask` surfaces (input before routing, output before the reply leaves) — **Tier 1** regex (`rules.py`, zero-LLM common path) + **Tier 2** haiku LLM-judge (`judge.py`) escalating **only the ambiguous band** (a broad **suspicion net**), not every turn (D-003 latency), **fail-closed** on judge error. 3 categories (prompt-injection / game-jailbreak / toxicity); a block → **generic refusal** (no rule revealed — no info leak to a prober) + `guardrail.blocked` audit (operator/cross-tenant, no RLS — D-017; via a thin `app/services/guardrails.py` that keeps the LLM-judge call in the service layer). **The red-team gate is real:** `data/eval/redteam.jsonl` (47 records, diverse techniques + benign controls), `eval_thresholds.yaml max_successful_injections=0`, harness compares **directly** (no `_min`/`_max` suffix — `passes_threshold` would raise); `tests/test_eval_redteam.py -m redteam` runs the **real judge**, deselected from default CI, gated in `eval-redteam.yml` (needs the `ANTHROPIC_API_KEY` secret). **Honest convergence 13→2→1→1→0** (stable ×3 real-judge runs): run 1 found **13 slips — every one the suspicion net failing to ESCALATE** (the judge blocked everything that reached it; the deterministic-first coverage risk materialized AND was caught by the gate — its self-checking property), fixed by widening the nets + generalizing Tier-1 patterns + strengthening the judge prompt — **general classes, not the verbatim set strings**; **0 over-blocks throughout** (precision held). **327 tests green** + 1 deselected. **6.2 delivered (right-to-erasure VERIFY — not a rebuild):** erasure was built (4.1b) + 3-of-4 D-032 guarantees already tested (Postgres physical-deletion owner-connection, Redis cleared, `tenant.erased` audit, + guest erasure); the **one gap** was D-032's "preferences RETAINED" (asserted, untested) — closed by `test_delete_me_retains_preferences` (PATCH prefs → erase → owner-connection `tenant_preferences==1` + post-erasure `GET /me/preferences` still returns it). 6.1 added **no new tenant-scoped content** to purge — `guardrail.blocked` is operator-audit (no PII, retained by design like all audit). **Live in-stack capture:** player + prefs + 4 msgs/2 sessions/2 Redis keys → `DELETE /me` → content **0**, Redis cleared, `tenant.erased deleted_rows=6`, **prefs + account retained**. SECURITY §3/§4/§6 reconciled. **328 tests green.** **6.3 delivered (CI eval gates — Section 6 done):** the audit found the gates already wired/enforced (`eval-redteam.yml` auto + proven green in 6.1; `eval-rag.yml` genuinely `_assert_thresholds`-raises → job red, manual-dispatch as the corpus DB is too heavy per-PR; all thresholds are REAL measured numbers) — so 6.3 = the two **demonstrations**. **P-016 closed (commit 1):** `app/eval/agent/harness.py` + a 12-question progression set → the agent grounds **12/12 = 100%** (objective metric: it called a live-state tool, inspected from `result['messages']`; the agent reaches live state only via the tools) + a class-distinct melee↔ranger spot-check; **measured, NOT re-tuned** (4.4's n≈5 → 12/12 across 3 classes × 3 stages). EVALS §3b; a recorded number, not a CI gate. **Regression-turns-red PROVEN (commit 2, Project Rule 4):** disabled the Tier-2 judge → red-team harness **21 successful (RED)** → revert → **0 (green)** — the 9 Tier-1-caught attacks still block, so the judge catches the 21 the deterministic tier misses (load-bearing) AND the gate flips red; break reverted, no permanent code (EVALS §2.5). `eval-rag` enforces identically (the `_assert_thresholds` raise → `pytest -m eval` non-zero → job red — no need to physically break RAG). `ci.yml` tightened `mypy app`→`mypy app tests scripts`. EVALS §5 Final Numbers filled. **328 tests green.** **Section 6 COMPLETE.** **7.1 delivered (Section 7 — Polish & Present opens; the first NON-GRADED phase — demo presentation, no new scored functionality):** a UI polish pass on both web surfaces, one shared **Everforest** palette so they read as one product, in **2 commits**. **Commit 1 (React portal):** a runtime light/dark **theme picker** (light-hard/dark-hard, default dark, `localStorage`-persisted, `data-theme` before first paint) + spacing/type/state polish + brand monogram/favicon (`npm run build` green — the gate; no frontend CI). **Commit 2 (Streamlit bench):** Everforest **dark-only** (`.streamlit/config.toml` — Streamlit themes are **startup-only**, no runtime toggle → **D-035**: portal carries the picker, bench gets a single coherent dark theme matching the portal's dark mode) + UX (preset **detail table** not a JSON blob, a new **"Fresh start" n≈0 preset** listed first, **Versions tab** reordered — re-rag on top + versions as a table, `source_chunks` as a table, **"Reset chat"** relabel + a confirmation toast). **No backend changes** — visual/data-display only (`api.*` calls, operator gating, the re-rag polling fragment, `/bot/ask`, the preset→payload wiring all intact); scoped `frontend-admin` rebuilds throughout (backend never touched); both surfaces verified in-browser, `ci.yml` (backend-only) just confirms green (nothing to regress). **328 tests green.** **Next: 7.2 — live demo re-runs (isolation + erasure + guardrail, re-shown live for the defense).**
