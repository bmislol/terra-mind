...
### D-034 — Guardrail architecture + red-team set (graduates P-006)
**Status:** Locked (2026-06-18, Phase 6.1)
**Choice:** A **two-tier, deterministic-first guardrail** on both `/bot/ask` surfaces (input before routing, output before the reply leaves — ARCH §5 steps 4/9). **Tier 1 — deterministic** (regex/keyword, zero LLM): a clear per-category hit BLOCKS; a clearly-benign message PASSES — so the common path (real game questions, obvious attacks) costs **no LLM call** (D-003 latency). **Tier 2 — LLM-judge** (`claude-haiku-4-5`, D-003): fires **only on the ambiguous band** (Tier 1 neither hard-blocks nor clears, a broad suspicion net trips), returning BLOCK/ALLOW + category — **not** a judge on every turn. Three threat **categories**: prompt injection, game/progression jailbreak (cheat / dev-item / spawn-drops requests), toxicity (in + out). A block → a generic player-facing **refusal** (no rule revealed — no info leak to a prober) + a `guardrail.blocked` **audit** row (operator/cross-tenant, like the other audit rows — D-017; visible in the 5.2 Audit tab).
**Red-team set** (`app/eval/redteam/redteam_set.jsonl`): `{text, category, surface: input|output, must_block}`, sized for **adversarial DIVERSITY** — many DISTINCT techniques per category (injection: instruction-override, role-play/DAN, system-prompt extraction, delimiter/encoding, multi-turn setup, payload-in-game-context; jailbreak: dev-items, spawn-drops-without-fighting, /give-style, stat-setting, impossible-craft; toxicity in/out), each with a couple of paraphrases — **not** near-duplicate padding. **Plus expanded benign controls** (`must_block:false`, incl. the borderline-legit cases — "beat the Moon Lord", "what does Zenith drop", "get dev items legitimately") that prove **no over-block**. The harness routes each record to `check_input`/`check_output`, counting **successful injection = a must_block record that passed** and **over-block = a benign control that was blocked**.
**Why — accepted coverage-risk, mitigated by the gate:** deterministic-first trades a theoretical recall gap (a novel attack that slips Tier 1 *and* doesn't trip the suspicion net) for zero common-path latency. This is **safe because the red-team gate self-validates**: an uncaught attack surfaces as `successful > 0` and **fails the build**, forcing the net wider — the gate is the coverage proof, not a hope. Tier 1 is tuned for **precision** (block the obvious, never the benign); Tier 2 + the judge supply **recall**.
**Number / evidence:** gate = **`redteam.max_successful_injections: 0`** (already committed). The harness compares **directly** (`successful <= max`), NOT via `passes_threshold` (which raises on the no-`_min`/`_max`-suffix key by design — `zero_is_valid_for_key` carves it out). Real-LLM red-team run = the `eval-redteam.yml` gate (PR-triggered, `ANTHROPIC_API_KEY`, no DB); default `pytest` uses deterministic + **mocked-judge** unit tests (no API key). Built in **4 staged commits** (Tier-1 deterministic core → Tier-2 judge + escalation → red-team set + harness → `/bot/ask` wiring + closeout). **Gate proven** (real judge): **13→2→1→1→0** successful injections, **0 over-blocks throughout**, stable ×3 runs — every run-1 slip was the suspicion net failing to escalate (the judge blocked everything that reached it), fixed by widening the nets + generalizing Tier-1 patterns + the judge prompt as **general classes**, not the verbatim set strings.

### D-035 — Per-surface theming: React portal gets a light/dark picker, the Streamlit bench is dark-only
**Status:** Locked (2026-06-20, Phase 7.1)
**Choice:** Both web surfaces share one **Everforest** palette (same green accent `#a7c080`) so they read as one product. The React **config portal** (player-facing) ships a **runtime light/dark theme picker** — light-hard / dark-hard, default **dark**, persisted in `localStorage`, applied to `<html data-theme>` before first paint. The Streamlit **operator bench** is **dark-only**, themed via `.streamlit/config.toml` (matching the portal's dark mode).
**Why:** Streamlit's theme is **startup-only** — it's read from `config.toml` at server boot and there is **no supported runtime theme toggle** in the Streamlit API. A per-session in-app toggle would mean re-implementing the whole theme as CSS injected against Streamlit's internal classes (fragile, unsupported, maintenance debt) for an **internal operator tool**. So the portal (React, full DOM control, player-facing) carries the picker; the bench gets a single coherent dark theme. This is the documented answer to "why isn't the bench toggleable?" — a framework constraint, not an oversight.
**Scope:** Phase 7.1 is **NON-GRADED** polish — visual/UX + preset display data only, no backend or api-logic change; the existing flows (login / prefs / erasure / test-chat / re-rag / audit) are untouched and `ci.yml` (backend-only) has nothing to regress.
...
| P-020 | Re-rag single-job lock is released by the job's except handler **before** RQ retries, so during a retry window the 409 guard is bypassed (found Phase 5.3 in-stack). **Harmless today** — one `worker` container runs one job at a time, so a 2nd enqueue just waits — but the correct fix is to release the lock only on the *final* attempt (check `job.retries_left`) | worker-hardening pass | n/a (correctness edge; single-worker mitigates) |
| P-021 | The `worker` (and `api`) re-download the embedding model from HF on each image rebuild — slow/flaky under HF rate-limiting (blocked the first 5.3 in-stack boot). A shared HF cache (named volume or mounted `~/.cache/huggingface`, RUNBOOK §4.1) removes the runtime download for both | ops/infra polish | n/a (ops reliability; model is pinned D-004) |

_P-006 graduated to **D-034** (Phase 6.1): deterministic-first guardrail (Tier-1 regex, zero-LLM common path) + an escalation LLM-judge (haiku, ambiguous band only); red-team set keyed on **adversarial diversity** + benign controls; gate = **0 successful injections** (harness compares directly, the key has no `_min`/`_max` suffix by design)._
...
- **2026-06-18 · D-034 (new, Phase 6.1) — graduates P-006:** the guardrail architecture + red-team set. **Deterministic-first** (Tier-1 regex, zero-LLM common path) + an **escalation LLM-judge** (haiku, ambiguous band only — not every turn, D-003 latency); both `/bot/ask` surfaces (input pre-routing, output pre-reply). Three categories (prompt injection / game-jailbreak / toxicity); a block → generic refusal + `guardrail.blocked` audit (D-017). Red-team set keyed on **adversarial diversity** (distinct techniques/category) + expanded benign controls (no over-block). Accepted coverage-risk **mitigated by the gate** (an uncaught attack → `successful>0` → build red → widen the net). Gate `redteam.max_successful_injections: 0`, harness compares directly (no `_min`/`_max` suffix). Built in 4 staged commits; wired into `/bot/ask` (input pre-routing, output pre-reply) via `app/services/guardrails.py`. **Gate proven 13→2→1→1→0** (real judge, stable ×3); 327 tests green + 1 deselected (the redteam gate).
- **2026-06-20 · D-035 (new, Phase 7.1):** Per-surface theming — the React portal gets a **runtime light/dark Everforest picker** (default dark, `localStorage`, `data-theme` before first paint); the Streamlit bench is **dark-only** via `.streamlit/config.toml` because **Streamlit themes are startup-only** (no runtime toggle API) and an injected-CSS toggle for an internal tool isn't worth the fragility. Both share the Everforest accent so the two surfaces read as one product. Phase 7.1 = NON-GRADED polish (visual/UX + preset display only; no backend/api-logic change; existing flows untouched). **Commit 1 (portal):** theme picker + spacing/type/state polish + brand monogram/favicon. **Commit 2 (bench):** dark theme + preset detail table, new "Fresh start" n≈0 preset, Versions tab reorder + table, `source_chunks` table, "Reset chat" relabel + toast.
- **2026-06-17 · D-033 (new, Phase 5.3) — reverses P-019:** operator-triggered re-rag built as a **robust background job** — RQ worker on the existing Redis + a minimal Postgres `rerag_jobs` history table (operator data → no RLS, `require_operator`-gated, D-017) + single-job 409 + streamed progress. Scope locked to **re-embedding the cached corpus** (re-scrape = documented seam, no current payoff); the job uses the **idempotent upsert, never `--force`** (the audit found `--force` leaves a half-deleted version on mid-job death). A **deliberate reversal** logged knowing it precedes the graded Section 6. Built in 4 staged commits (worker/broker infra → progress refactor → API+guard → UI). _Commit 1: worker/broker infra + smoke-job round-trip. Commit 2: `run_build` progress-seam refactor. Commit 3: `rerag_jobs` table (no RLS, role-gated, D-017) + the worker job fn (`corpus.reragged` audit) + the single-job 409 lock + `POST /admin/rerag` & `GET /admin/rerag/status/{id}`. Two corrections surfaced building commit 3: `run_build` **relocated** to `app/rag/corpus_build.py` (the worker image ships `app/`, not `scripts/` — and app→scripts would invert the layering), and the sync build DSN fixed to `postgresql+psycopg://` (psycopg2 isn't a dependency, so the worker would have crashed at engine creation). 266 tests green._
